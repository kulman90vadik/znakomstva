...
const Card = ({ item }: Props) => {
  ...
  const {mutate} = useMutation({ // useMutation - для всего кроме GET!
    ...
    onSuccess() { // если всё хорощо прошло то очищаем
      queryClient.invalidateQueries({queryKey: ['list']}) // для автоматического обновления данных.
    }
  })
  ...
  const clickHandler = async (id: number, obj: usersType) => {
    mutate({...obj, favorites: !obj.favorites })
      // const res = await fetch(`https://contact-server-pd98.onrender.com/znakomstva/${id}`);
  ...
  return (
    <>
      <Link
        className="relative z-10 after:absolute after:content-* after:inset-0 after:transition
        after:duration-200 after:ease-linear after:rounded-[30px]after:bg-black-300
        hover:after:bg-black-200"
        to={`/person/${item.gender}/${item.name}`}
        state={{ id: item.id }}
      >
        <img
      ...
      <div className="absolute bottom-0 left-0  w-full z-20">
        <div className="pl-20 pr-20 flex items-center w-full mb-10">
          <div className="flex mr-10 font-semibold text-[21px] uppercase">
            {item.name}
            {item.status && (
      ...
      <button
        ...
        className={`absolute z-20 top-10 left-10 flex justify-center items-center size-40 bg-buccaneer/30
        hover:bg-buccaneer duration-300 transition-colors ease-in-out rounded-[50%]
        ${item.favorites ? "activeBtn" : ""}`}>
